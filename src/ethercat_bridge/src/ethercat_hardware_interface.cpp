...
// often not an EtherCAT PDO device at all (typically a separate serial or CAN
// link), so this stays a fixed placeholder, independent of the
  ...
  const hardware_interface::HardwareInfo & info)
{
  ...
  for (size_t i = 0; i < n; ++i)
  {
    for (const auto & state_if : info_.joints[i].state_interfaces)
    {
      ...
      {
        // Humble's InterfaceInfo exposes <param name="initial_value"> as a plain
        // string member (empty if unset); the generic `parameters` map is a
        // later ros2_control release. Same access mujoco_system.cpp uses.
        if (!state_if.initial_value.empty())
        {
          try
          {
            position_state_[i] = std::stod(state_if.initial_value);
          }
          catch (const std::exception &)
          {
            RCLCPP_WARN(
              logger(), "joint '%s': could not parse initial_value '%s', using 0.0",
              joint_names_[i].c_str(), state_if.initial_value.c_str());
          }
        }
      }
    }
...
bool EthercatHardwareInterface::load_and_validate_slave_config(const std::string & path)
{
  ...
  for (size_t i = 0; i < slaves.size(); ++i)
  {
    ...
    // Compared as strings, not parsed as hex integers, to avoid depending
    // on yaml-cpp's hex-literal numeric conversion behavior, which
  ...
  const rclcpp_lifecycle::State &)
{
  ...
  // Verify it against the real drive datasheet before implementing this loop
  // and do not trust a copy-pasted generic mask.
  RCLCPP_WARN(
    logger(),
  ...
  const rclcpp::Time &, const rclcpp::Duration &)
{
  ...
  // Static on purpose instead of a fabricated physics model: this class must
  // not pretend to simulate a robot. That job belongs to
