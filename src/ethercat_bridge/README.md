...
- `EthercatHardwareInterface` is a genuine, loadable `pluginlib` plugin.
...
| IMU (`imu_imu` sensor interfaces) | Read a real pelvis IMU | Hardcoded identity orientation, zero gyro/accel — a real pelvis IMU is not necessarily even an EtherCAT device (often a separate serial/CAN link), so this is tracked as its own open item, not conflated with the PDO TODOs above |
